...
import CharacterTypeSelectionScreen from "./CharacterTypeSelectionScreen/CharacterTypeSelectionScreen";
import ElementSelectionScreen from "./ElementSelectionScreen/ElementSelectionScreen";
import GameScreen from "./GameScreen/GameScreen";

const makeComputerChoice = () => {
  let computerNumber = Math.floor(Math.random() * 2);
  const lastComputerChoice = String(
    sessionStorage.getItem("lastComputerChoice") || "An Error"
  );
...
const checkThisPlayersHealthLost = (
  thisPlayersChoice: string,
  theOtherPlayersChoice: string
) => {
  if (thisPlayersChoice === "Shield" && theOtherPlayersChoice === "Shield") {
    return 0;
  } else if (
    thisPlayersChoice === "Shield" &&
    theOtherPlayersChoice === "Magic"
  ) {
    return 1;
  } else if (
    thisPlayersChoice === "Shield" &&
    theOtherPlayersChoice === "Sword"
  ) {
    return 0;
  } else if (
    thisPlayersChoice === "Magic" &&
    theOtherPlayersChoice === "Shield"
  ) {
    return 0;
  } else if (
    thisPlayersChoice === "Magic" &&
...
const checkBonusElementDamage = (
  thisPlayersElement: string,
  theOtherPlayersElement: string
) => {
  if (thisPlayersElement === "Fire" && theOtherPlayersElement === "Earth") {
    return 1;
  } else if (
    thisPlayersElement === "Earth" &&
    theOtherPlayersElement === "Air"
  ) {
    return 1;
  } else if (
    thisPlayersElement === "Air" &&
    theOtherPlayersElement === "Water"
  ) {
    return 1;
  } else if (
    thisPlayersElement === "Water" &&
    theOtherPlayersElement === "Fire"
  ) {
    return 1;
  } else if (
    thisPlayersElement === "Light" &&
    theOtherPlayersElement === "Dark"
  ) {
    return 1;
  } else if (
    thisPlayersElement === "Dark" &&
    theOtherPlayersElement === "Light"
  ) {
    return 1;
  } else {
    return 0;
...
const OnlyAppRoute: React.FC = () => {
  const [screenName, setScreenName] = useState("Shield-Magic-Sword");

  const [playerOneCharacterType, setPlayerOneCharacterType] =
    useState("None Yet");
  const [computerCharacterType, setComputerCharacterType] =
    useState("None Yet");
  ...
  if (playerOneChoice !== "First Turn") {
    ...
    const playerOneHealthLost = checkThisPlayersHealthLost(
      playerOneChoice,
      computerChoice
    );
    ...
    if (
      ...
      playerOneCharacterType === "Bodyguard" &&
    ...
    if (computerChoice === "Magic") {
      computerElementalBonusDamage = checkBonusElementDamage(
        computerElement,
        playerOneElement
      );
    }
    ...
    const computerHealthLost = checkThisPlayersHealthLost(
      computerChoice,
      playerOneChoice
    );
    ...
    if (playerOneChoice === "Magic") {
      playerOneElementalBonusDamage = checkBonusElementDamage(
        playerOneElement,
        computerElement
      );
    }
  ...
  const chooseCharacterTypeButtonPressed = (characterTypeButtonSettings: {
    ...
    setPlayerOneCharacterType(
      characterTypeButtonSettings.playerOneCharacterType
    );
